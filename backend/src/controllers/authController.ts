...
export async function register(c: Context) {
    ...
    try {
        ...
        const user = await userService.registerWithEmailPassword(email, password, name, role);
 ...
 * Body: { phone: string, otp: string, name?: string, role?: string }
...
export async function verifyPhoneOTP(c: Context) {
    ...
    try {
        ...
        const user = await userService.findOrCreateUserByPhone(phone, name, role);
